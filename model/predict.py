...
# Function for cropping and prediction (copy-pasted from original code)
def increased_crop(img, bbox : tuple, bbox_inc : float = 1.5):
    real_h, real_w = img.shape[:2]
...
def predict():
    ...
    # Predict using the models and functions
...
orb = cv2.ORB_create()
reference_img = cv2.imread("image.png")
...
def crop_and_resize(img, bbox: tuple, scale: float = 1.5):
    h, w = img.shape[:2]
    x, y, width, height = bbox
    center_x, center_y = x + width // 2, y + height // 2

    # Scale the bounding box
    length = int(max(width, height) * scale)
    x1, y1 = max(0, center_x - length // 2), max(0, center_y - length // 2)
    x2, y2 = min(w, center_x + length // 2), min(h, center_y + length // 2)
...
def make_prediction(img, face_detector, anti_spoof):
    ...
    # Convert bboxes to a NumPy array if it is a list
...
def predict():
    ...
    # Run face detection and anti-spoof prediction
    result = make_prediction(img, face_detector, anti_spoof)
...
# Load reference image for face matching
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
orb = cv2.ORB_create()
reference_img = cv2.imread("image.png")
gray_reference = cv2.cvtColor(reference_img, cv2.COLOR_BGR2GRAY)
...
def match():
    ...
    match_found = any(executor.submit(match_faces, gray_img[y:y+h, x:x+w]).result() for (x, y, w, h) in faces)
...
def match():
    ...
    bbox, label, score, cropped_face = make_prediction(img, face_detector, anti_spoof)
    
    if label == 'FAKE' or cropped_face is None:
